...
import FileUploader from "../../../../ui/Upload/FileUploader";
  ...
  return (
    <div className={styles.quizQuestion}>
      <input
        ...
        onChange={(e) =>
          onChange(questionIndex, { ...questionData, question: e.target.value })
        }
        required
...
const LessonFormWithContent = ({ modules = [], onNext }) => {
  ...
  const handleFileUpload = (fileData) => {
    ...
    // Optionally, associate fileData.secure_url with the lesson form state
  };
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      if (contentType === "video")
        lessonPayload.content_url = contentUrl.trim();
      else if (contentType === "text")
        lessonPayload.content_text = contentText.trim();
      ...
      if (!lessonData.success)
        throw new Error(lessonData.message || "Failed to add lesson.");
      ...
      if (contentType === "quiz") {
        for (const q of quizQuestions) {
          ...
          if (q.options.some((opt) => !opt.trim()))
            throw new Error("All quiz options must be filled.");
          ...
          if (!quizData.success)
            throw new Error(quizData.message || "Failed to add quiz question.");
        }
      }
      ...
      if (contentType === "assignment") {
        ...
        if (!assignmentData.success)
          throw new Error(
            assignmentData.message || "Failed to add assignment."
          );
      }
        ...
        {contentType === "video" && (
          <>
            <input
              ...
              required
            />
            {/* FileUploader for video files/materials */}
            <div style={{ margin: "10px 0" }}>
              <FileUploader lessonId={null} onUpload={handleFileUpload} />
